...
class Storage:
    ...
    @contextmanager
    def file_lock(self, path: str | Path):
        lock_path = Path(path)
    ...
    @contextmanager
    def staged_directory(self, target: str | Path):
        target = Path(target)
        target.parent.mkdir(parents=True, exist_ok=True)
        temporary = Path(tempfile.mkdtemp(prefix=f".{target.name}.", dir=target.parent))
        try:
            yield temporary
            with self.file_lock(target.parent / f"{target.name}.lock"):
                if os.path.lexists(target):
                    raise ConflictError(f"already exists: {target}")
                try:
                    os.rename(temporary, target)
                except FileExistsError as exc:
                    raise ConflictError(f"already exists: {target}") from exc
                except OSError as exc:
                    raise OperationError(f"could not commit staged directory {target}: {exc}") from exc
        finally:
            if temporary.exists():
    ...
    def atomic_write_json(self, path, obj):
        return self.atomic_write_text(path, json.dumps(obj, indent=2, sort_keys=True) + "\n")
    ...
    def read_json(self, path):
        try:
            ...
            raise ValidationError(f"invalid JSON: {path}") from e
    ...
    def exclusive_write_json(self, path, obj):
        ...
        try:
            ...
            try:
                ...
                raise ConflictError(f"already exists: {path}")
        finally:
            if os.path.exists(tmp):
                os.unlink(tmp)
        return path
    ...
    def resolve_ship(self, explicit=None):
        if explicit is not None:
            ...
            if (path / "metadata.json").exists() and (path / "index.md").exists():
                return path
            raise NotFoundError(f"ship not found: {path}")
        candidates = []
        if os.environ.get("CAPTAIN_BRIDGE_SHIP"):
            candidates.append(Path(os.environ["CAPTAIN_BRIDGE_SHIP"]))
        p = Path.cwd()
        candidates.extend([p, *p.parents])
        for candidate in candidates:
            candidate = candidate.expanduser().resolve()
            if (candidate / "metadata.json").exists() and (candidate / "index.md").exists():
                return candidate
